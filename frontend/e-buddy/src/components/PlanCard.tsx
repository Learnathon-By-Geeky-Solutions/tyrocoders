...
export function PlanCard({
  ...
  return (
    <Card className={`pricing-card w-full ${isPrimary ? 'border-2 border-accent relative' : ''}`}>
      {isPopular && (
      ...
      <CardHeader className={`pb-3 ${isPrimary ? 'bg-orange-50' : ''}`}>
        ...
        <div className="mt-1">
          <span className="text-2xl font-bold text-accent">${monthlyPrice}</span>
          <span className="text-gray-500 text-sm ml-1">/month</span>
        </div>
        {yearlyPrice && (
          <div className="text-sm text-gray-500 mt-1">
            ${yearlyPrice}/year <span className="text-accent">(Save 20%)</span>
          </div>
        )}
      ...
      <CardFooter className="border-t bg-gray-50/50 pt-4 pb-6">
        <Button 
          className={`w-full ${isPrimary ? 'bg-accent hover:bg-accent' : 'bg-accent hover:bg-accent'}`}
        >
          Choose {name}
        </Button>
      </CardFooter>
