...
export const ChatPreview = ({
  ...
  const handleSendMessage = () => {
    ...
    if (!convId) {
      console.error("No conversationId in sessionStorage!");
      setIsTyping(false);
      return;
    }

    chatAPI
      .continueConversation(convId, userInput)
      .then((res) => {
        // The API now returns:
        // { message: "...", data: { response, display_type, follow_up_question, conversation_type } }
        const botPayload = res.data.data;

        // Build a chat‐message using your existing createMessageContent helper
        const botMessage: ExtendedChatMessage = {
          id: Date.now().toString(),
          sender: "bot",
          timestamp: new Date(), // or you can embellish with server time
          content: createMessageContent({
            response: botPayload.response,
            display_type: botPayload.display_type,
            products: botPayload.products, // may be undefined for non‐product replies
            comparison_attributes: botPayload.comparison_attributes,
          }),
        };
            ...
            <div className="grid grid-cols-1 gap-2">
              {content.products.map((product, index) => (
                <a
                  ...
                  className="rounded-md overflow-hidden shadow-sm hover:scale-102 transition-scale duration-200"
                >
                  <div
                    key={product.id || index}
                    className="bg-white rounded-md overflow-hidden shadow-sm m-1"
                  >
                    {product.image && (
            ...
            <div className="space-y-4">
              {content.products.map((product) => (
                ...
                >
                  ...
                  {/* Accordion for Extra Attributes */}
  ...
  return (
    <div
      ...
      } bottom-5 shadow-lg rounded-lg overflow-hidden flex flex-col z-50 transition-all duration-300 ease-in-out max-w-xs w-full`}
      style={{
        fontFamily:
          ...
          <div className="bg-white flex-grow overflow-y-auto p-4 h-96">
            {messages.map((message) => (
              ...
              >
                <div
                  className={cn(
                    "max-w-[85%] p-3 animate-slide-in text-sm",
