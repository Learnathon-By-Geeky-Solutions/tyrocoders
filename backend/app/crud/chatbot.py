...
class ChatbotCrud():
    ...
    async def create_chatbot(self, chatbot: ChatbotCreate, user_id: str):
        collection = self.collection
        chatbot_dict = chatbot.model_dump(mode="json")
        chatbot_dict["user_id"] = str(user_id)
        # new_chatbot = await collection.insert_one({**chatbot_dict, "user_id": str(user_id)})
        new_chatbot = await collection.insert_one(chatbot_dict)
        return new_chatbot
    
    async def get_chatbot_by_id(self, chatbot_id: str, user_id: str):
        chatbot = await self.collection.find_one({"_id": ObjectId(chatbot_id), "user_id": user_id})
        return chatbot
