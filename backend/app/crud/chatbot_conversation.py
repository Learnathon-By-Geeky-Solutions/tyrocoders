...
class ChatbotConversationCrud():
    ...
    async def get_leads_by_chatbot_id(
        ...
        try:
            ...
            # Fetch all conversations matching the chatbot ID
            cursor = collection.find(query, {"_id": 0, "collect_lead_fields": 1})
            conversations = await cursor.to_list(length=None)

            leads = []
            for convo in conversations:
                collect_lead_fields = convo.get("collect_lead_fields", [])
                lead = {
                    field["field_name"]: field["field_value"]
                    for field in collect_lead_fields
                    if "field_name" in field and "field_value" in field
                }

                # Only append non-empty leads
                if lead:
                    leads.append(lead)
            ...
            raise RuntimeError(
                f"Error retrieving leads for chatbot ID {chatbot_id}: {e}"
            )